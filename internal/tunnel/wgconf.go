...
// Idempotent — safe to call on every Start() and every apply.
func (w *ConfWriter) EnsureInterface(iface InterfaceConfig) error {
	...
	// Apply base private key + listen port before syncconf to guarantee the device exists in the kernel.
	...
	if !strings.Contains(string(existingAddrs), TunnelGateway) {
		if out, err := exec.Command("ip", "addr", "add", TunnelGatewayCIDR, "dev", wgInterface).CombinedOutput(); err != nil && !strings.Contains(string(out), "File exists") {
	...
	if !strings.Contains(string(existingAddrs), "10.10.0.254") {
		_ = exec.Command("ip", "addr", "add", "10.10.0.254/32", "dev", wgInterface).Run() // best-effort
	}
...
func (w *ConfWriter) Apply(iface InterfaceConfig, peers []PeerConfig) error {
	...
	if err := w.EnsureInterface(iface); err != nil {
		return err
	}
...
func (w *ConfWriter) buildConf(iface InterfaceConfig, peers []PeerConfig) string {
	...
	sb.WriteString(fmt.Sprintf("PostUp = iptables -t nat -A POSTROUTING -s %s -o %s -j MASQUERADE; iptables -A FORWARD -i %s -j ACCEPT; iptables -A FORWARD -o %s -j ACCEPT\n", TunnelSubnet, natIface(), wgInterface, wgInterface))
	sb.WriteString(fmt.Sprintf("PostDown = iptables -t nat -D POSTROUTING -s %s -o %s -j MASQUERADE; iptables -D FORWARD -i %s -j ACCEPT; iptables -D FORWARD -o %s -j ACCEPT\n", TunnelSubnet, natIface(), wgInterface, wgInterface))
	sb.WriteString("\n")
	for _, p := range peers {
		if p.Comment != "" {
